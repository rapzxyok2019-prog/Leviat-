...
// --- 2. HOOK CUSTOMIZADO PARA LOCALSTORAGE ---
function useLocalStorage(key, initialValue) {
    const [storedValue, setStoredValue] = useState(() => {
        try {
            ...
            if (key === 'farm_monthly_history' && !Array.isArray(initialData)) {
                return initialValue;
            }
...
// --- 3. FUNÇÕES DE CÁLCULO E UTILITÁRIOS ---
...
function FarmDashboard() {
  ...
  },[memberCount, totals, setDelivered, viewingMemberIndex]); 
  ...
  if (loading) {
    return (
        <div className="text-center p-10 text-xl font-semibold text-indigo-700">
            Carregando Autenticação e Painel...
        </div>
    );
  }
  ...
  if (error) {
    return (
        <div className="text-center p-10 bg-red-100 border border-red-400 text-red-700 font-semibold rounded-lg">
            <h1 className="text-2xl mb-2">⚠️ Erro Crítico de Inicialização</h1>
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <h1 className="text-3xl font-bold text-indigo-800 mb-6 border-b-2 border-indigo-200 pb-2">Controle de Farm</h1>
